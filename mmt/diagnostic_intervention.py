...
def diagnostic_generate_with_interventions(
    ...
    def diagnostic_intervention_hook(module, inputs, output):
        if (
            hasattr(diagnostic_intervention_hook, "active")
            and diagnostic_intervention_hook.active
        ):
            ...
            if len(output.shape) == 3:
                ...
                if feature_vector.shape[0] != d_model:
                    print(
                        f"      ❌ DIMENSION MISMATCH: feature {feature_vector.shape[0]} vs output {d_model}"
                    )
                    return output
                ...
                intervention_stats["activation_norms_before"].append(
                    activation_norm_before
                )

                if intervention_type == "addition" and strength != 0.0:
                    print(f"      ➕ Applying ADDITION with strength {strength}")
                    output[:, -1, :] = (
                        last_token_before + strength * feature_vector.unsqueeze(0)
                    )
                    ...
                    intervention_stats["interventions_applied"] += 1
                    intervention_stats["feature_projections"].append(
                        projection_coeff.item()
                    )
                else:
                ...
                intervention_stats["activation_norms_after"].append(
                    activation_norm_after
                )

                change_magnitude = torch.norm(
                    last_token_after - last_token_before
                ).item()
                print(
                    f"      📊 Activation norm: {activation_norm_before:.6f} → {activation_norm_after:.6f}"
                )
    ...
    # Try to hook the actual computation layers (not container layers)
    layer_patterns = [
        f"decoder.net.attn_layers.layers.{intervention_layer}.1",  # The feedforward layer
        f"decoder.net.attn_layers.layers.{intervention_layer}.0",  # The attention layer
        f"decoder.net.attn_layers.layers.{intervention_layer}.1.net",  # FF network
        f"decoder.net.attn_layers.layers.{intervention_layer}.0.0",   # Attention computation
    ]
    ...
    if intervention_stats["activation_norms_before"]:
        avg_norm_before = sum(intervention_stats["activation_norms_before"]) / len(
            intervention_stats["activation_norms_before"]
        )
        avg_norm_after = sum(intervention_stats["activation_norms_after"]) / len(
            intervention_stats["activation_norms_after"]
        )
        print(f"   Avg activation norm: {avg_norm_before:.6f} → {avg_norm_after:.6f}")
    if intervention_stats["feature_projections"]:
        avg_projection = sum(intervention_stats["feature_projections"]) / len(
            intervention_stats["feature_projections"]
        )
        ...
        for name, sequence in all_results.items():
            if name != "baseline":
                if torch.equal(baseline_seq, sequence):
                    ...
                    print(
                        f"   ✅ {name}: {diff_tokens}/{total_tokens} tokens different ({100*diff_tokens/total_tokens:.1f}%)"
                    )
