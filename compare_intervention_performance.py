...
def compute_intervention_effect(baseline_values: List[float], intervention_values: List[float]) -> Dict[str, float]:
...
def analyze_feature_interventions(results: Dict) -> Dict:
    ...
    extended_metrics = [
        "note_density", "notes_per_beat", "total_notes",
        "pitch_range", "mean_pitch", "pitch_std", 
        "rhythmic_regularity", "tempo_stability",
        "harmonic_consistency", "chord_progression_quality",
        "structural_coherence", "melodic_coherence"
    ]
    ...
    for result in results["detailed_results"]:
        ...
        if "baseline" in result["conditions"]:
            ...
            for metric_name in all_metrics:
                baseline_metrics[metric_name] = extract_metric_values(baseline_condition, metric_name)
        ...
        for condition_name, condition_data in result["conditions"].items():
            ...
            for metric_name in all_metrics:
                ...
                if baseline_values and intervention_values:
                    effect_stats = compute_intervention_effect(baseline_values, intervention_values)
...
def analyze_strength_effects(feature_analyses: Dict) -> Dict:
    ...
    for feature_key, feature_data in feature_analyses.items():
        for condition_name, condition_analysis in feature_data["condition_analyses"].items():
            ...
            if condition_info.get("condition_type") == "addition":
                ...
                for metric_name, effect_stats in condition_analysis["metric_comparisons"].items():
                    strength_effects[metric_name][strength].append(effect_stats["percent_change"])
...
def assess_model_performance_preservation(feature_analyses: Dict) -> Dict:
    ...
    performance_metrics = ["pitch_class_entropy", "scale_consistency", "groove_consistency"]
    ...
    for feature_key, feature_data in feature_analyses.items():
        ...
        for condition_name, condition_analysis in feature_data["condition_analyses"].items():
            condition_type = condition_analysis["condition_info"].get("condition_type", "unknown")
            ...
            for metric_name in performance_metrics:
                if metric_name in condition_analysis["metric_comparisons"]:
                    ...
                    preservation_analysis["by_condition_type"][condition_type][metric_name].append(degradation)
                    preservation_analysis["by_layer"][layer][metric_name].append(degradation)
    ...
    for metric_name, degradations in all_degradations.items():
        if degradations:
            preservation_analysis["overall_preservation"][metric_name] = {
                ...
                "preservation_score": float(100.0 / (1.0 + np.mean(degradations))),  # Higher = better preservation
                "count": len(degradations)
            }
    
    # Degradation summary by condition type
    for condition_type, metrics_data in preservation_analysis["by_condition_type"].items():
        condition_summary = {}
...
def generate_summary_recommendations(comparative_analysis: Dict) -> List[str]:
    ...
    performance_analysis = comparative_analysis.get("model_performance_preservation", {})
    overall_preservation = performance_analysis.get("overall_preservation", {})
    
    avg_preservation = np.mean([
        metrics["preservation_score"] for metrics in overall_preservation.values()
        if "preservation_score" in metrics
    ]) if overall_preservation else 50.0
    
    if avg_preservation > 80:
        recommendations.append("✅ Excellent model performance preservation across interventions")
    elif avg_preservation > 60:
        recommendations.append("⚠️ Good model performance preservation with some degradation")
    else:
    ...
    if strength_analysis:
        ...
        for metric_name, strength_data in strength_analysis.items():
            ...
            if len(strengths) >= 2:
                changes = [strength_data[s]["mean_change"] for s in strengths if s in strength_data]
                if len(changes) >= 2:
                    # Check for monotonic relationship
                    correlation = np.corrcoef(strengths, changes)[0, 1] if len(strengths) == len(changes) else 0
                    if abs(correlation) > 0.7:
                        strong_effects.append(f"{metric_name} (r={correlation:.2f})")
        
        if strong_effects:
            recommendations.append(f"📈 Strong strength-effect relationships found: {', '.join(strong_effects)}")
        else:
            recommendations.append("📊 Intervention strength effects are not strongly dose-dependent")
    ...
    if layer_analysis:
        ...
        for layer, metrics_data in layer_analysis.items():
            avg_magnitude = np.mean([
                metrics["effect_magnitude"] for metrics in metrics_data.values()
        ...
        if layer_effect_magnitudes:
            ...
            recommendations.append(f"🎯 Layer {max_layer[0]} shows strongest intervention effects ({max_layer[1]:.1f}% avg change)")
            recommendations.append(f"🎯 Layer {min_layer[0]} shows weakest intervention effects ({min_layer[1]:.1f}% avg change)")
        ...
        for feature_key, feature_data in feature_analysis.items():
            ...
            if total_comparisons > 0:
                ...
                if significance_rate > 0.5:
                    ...
                    significant_features.append(f"{feature_name} ({significance_rate:.1%} significant)")
        
        if significant_features:
            recommendations.append(f"🔍 Most effective interventions: {', '.join(significant_features)}")
        else:
            recommendations.append("🔍 Few statistically significant intervention effects detected")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Perform comparative analysis of intervention effects")
    parser.add_argument(
        "--extended-results",
        type=Path,
    ...
    comparative_analysis = {
        "analysis_info": {
            ...
            "analysis_types": [
                "feature_interventions", "strength_effects", 
                "layer_effects", "model_performance_preservation"
            ]
        },
    ...
    for metric_name, stats in model_performance["overall_preservation"].items():
        logger.info(f"  {metric_name}: {stats['preservation_score']:.1f}% preservation score")
