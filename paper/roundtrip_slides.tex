...
% ─────────────────────────────────────────────────────────────
\begin{frame}{Evaluation: Recovery Metric}

\textbf{Per-sample recovery:}
\begin{equation*}
    \text{Recovery}_i = 1 - \frac{|\text{W3}_{\text{RT},i} - \text{W3}_{\text{BL},i}|}{|\text{W2}_{\text{RT},i} - \text{W2}_{\text{BL},i}|}
\end{equation*}

\textbf{Aggregate recovery} (ratio of means, robust to outliers):
\begin{equation*}
    \text{Recovery}_\text{agg} = 1 - \frac{\overline{|\text{W3}_{\text{RT}} - \text{W3}_{\text{BL}}|}}{\overline{|\text{W2}_{\text{RT}} - \text{W2}_{\text{BL}}|}}
\end{equation*}

\vspace{0.5em}

\begin{itemize}
    \item W2 deviation = how far we steered (peak). W3 error = how much remains after recovery.
    \item 100\% = perfect recovery. 0\% = no recovery at all.
    \item Samples with negligible peak deviation filtered ($<$3 st, $<$1.5 tk)
\end{itemize}
...
\vspace{0.5em}
Per-sample aggregate:\\[3pt]
\color{pidblue}PID Recovery: \textbf{46\%}\color{black}\\[2pt]
\color{releasegreen}Release Recovery: 36\%\color{black}\\[2pt]
PID advantage: \textbf{+10 pp}
\end{column}
...
\vspace{0.5em}
Per-sample aggregate:\\[3pt]
\color{pidblue}PID Recovery: \textbf{66\%}\color{black}\\[2pt]
\color{releasegreen}Release Recovery: 40\%\color{black}\\[2pt]
PID advantage: \textbf{+26 pp}
\end{column}
\end{columns}

\vspace{1em}
\begin{center}
PID \textbf{always} outperforms passive release --- active back-steering recovers 10--26 pp more
\end{center}
...
\vspace{0.5em}
Per-sample aggregate:\\[3pt]
\color{pidblue}PID Recovery: \textbf{74\%}\color{black}\\[2pt]
\color{releasegreen}Release Recovery: 62\%\color{black}\\[2pt]
PID advantage: \textbf{+11 pp}
\end{column}
...
\vspace{0.5em}
Per-sample aggregate:\\[3pt]
\color{pidblue}PID Recovery: \textbf{15\%}\color{black}\\[2pt]
\color{releasegreen}Release Recovery: 7\%\color{black}\\[2pt]
PID advantage: \textbf{+8 pp} (2$\times$ better)
\end{column}
\end{columns}

\vspace{1em}
\begin{center}
Duration low: strongest scenario (74\% recovery). Duration high: weakest due to low peak deviation\\
(model resists downward changes), but PID still recovers \textbf{2$\times$ more} than passive release.
\end{center}
...
\begin{tabular}{llccccc}
    \toprule
    \textbf{Concept} & \textbf{Scenario} & \textbf{Peak Dev.} & \textbf{W3 Error} & \textbf{PID} & \textbf{Release} & \textbf{$\Delta$} \\
    \midrule
    \multirow{2}{*}{Pitch} & Low$\to$Up$\to$Down & 20.0 st & 10.8 st & \textbf{46\%} & 36\% & +10 pp \\
    & High$\to$Down$\to$Up & 34.7 st & 11.9 st & \textbf{66\%} & 40\% & +26 pp \\
    \midrule
    \multirow{2}{*}{Duration} & Low$\to$Up$\to$Down & 15.4 tk & 4.1 tk & \textbf{74\%} & 62\% & +11 pp \\
    & High$\to$Down$\to$Up & 9.3 tk & 7.9 tk & \textbf{15\%} & 7\% & +8 pp \\
    \midrule
    \multicolumn{4}{l}{\textbf{Average across all scenarios}} & \textbf{50\%} & 36\% & +14 pp \\
    \bottomrule
\end{tabular}
\end{center}
...
\textbf{Key takeaways:}
\begin{enumerate}
    \item PID recovers \textbf{15--74\%} of induced deviation (average 50\%)
    \item Recovery is \textbf{actively driven} --- PID outperforms release by 8--26 pp in every scenario
    \item Three scenarios show strong recovery (46--74\%); duration-high is weakest (low peak $\Rightarrow$ low signal)
    \item PID advantage is \textbf{largest where steering is strongest} (pitch high: +26 pp)
\end{enumerate}
...
\begin{itemize}
    \item The intervention is \textbf{partially reversible} (46--74\% recovery for 3/4 scenarios)
    \item Recovery is \textbf{active}, not passive --- PID outperforms release by 8--26 pp
...
\begin{itemize}
    ...
    \item Asymmetric difficulty: recovering from \emph{weak} steering (duration-high) gives less signal to guide recovery
\end{itemize}

\vspace{0.5em}
\textbf{46--74\% recovery from a single PID pass, always beating passive release, is strong evidence of controllable, reversible steering.}
\end{column}
\end{columns}
