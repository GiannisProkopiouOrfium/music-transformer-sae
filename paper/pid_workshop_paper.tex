...
A unique capability of temporal PID is \emph{reversible steering}: steer away from a conditioned prefix, hold, then steer back toward original characteristics---impossible with static SAS, which applies a fixed $\lambda$ throughout. We evaluate this with a three-phase schedule per sample: Phase~1 (64 tokens, steer away), Phase~2 (64 tokens, hold), Phase~3 (64 tokens, steer back), using 16-beat conditioned prefixes from extreme songs ($n{=}20$ per scenario). Full results are in \cref{app:roundtrip}.

For pitch (low$\to$up$\to$down), the round-trip trajectory follows the expected pattern: Window~1 shifts pitch from 49.7 to 59.5~st, Window~2 peaks at 74.0~st, and Window~3 recovers to 57.2~st---substantially closer to the unsteered baseline (59.6~st) than static one-way steering, which remains stuck at 73.4~st. The recovery error (Window~3 distance from baseline) is $37\%$ lower for PID (10.0 vs.\ 15.7~st), with $62\%$ of samples recovering to within $1\sigma$ of baseline.
Duration shows similar recovery: for low$\to$up$\to$down, PID reduces Window~3 error by $43\%$ (14.2 vs.\ 24.8~ticks). These results demonstrate that temporal PID enables compositional control trajectories that static methods fundamentally cannot express.
...
\cref{tab:roundtrip_pitch,tab:roundtrip_duration} report per-window attribute values for the round-trip experiment (\cref{sec:roundtrip}). Each scenario uses 10 extreme songs $\times$ 2 repetitions ($n{=}20$).

\begin{table}[h]
  \caption{Round-trip pitch steering: per-window mean pitch (st). Recovery error is $|\text{W3} - \text{baseline W3}|$.}
  ...
  \begin{tabular}{llccc}
    ...
    \midrule
    \multirow{3}{*}{Low$\to$Up$\to$Down}
    & Baseline & 49.70 & 58.62 & 59.63 \\
    & Round-trip PID & 59.51 & 73.95 & 57.21 \\
    & Static one-way & 62.12 & 73.42 & 73.40 \\
    \midrule
    & \multicolumn{2}{l}{Recovery error (PID / Static):} & \multicolumn{2}{c}{10.0 / 15.7 st} \\
    \midrule
    \multirow{3}{*}{High$\to$Down$\to$Up}
    & Baseline & 81.41 & 79.25 & 78.42 \\
    & Round-trip PID & 62.34 & 39.02 & 48.79 \\
    & Static one-way & 57.04 & 39.15 & 39.69 \\
    \midrule
    & \multicolumn{2}{l}{Recovery error (PID / Static):} & \multicolumn{2}{c}{27.1 / 33.4 st} \\
    \bottomrule
  \end{tabular}
  \end{small}
  \end{center}
...
\begin{table}[h]
  \caption{Round-trip duration steering: per-window mean duration (ticks).}
  ...
  \begin{tabular}{llccc}
    ...
    \midrule
    \multirow{3}{*}{Low$\to$Up$\to$Down}
    & Baseline & 2.31 & 2.41 & 2.44 \\
    & Round-trip PID & 11.25 & 25.31 & 17.88 \\
    & Static one-way & 14.38 & 26.45 & 26.07 \\
    \midrule
    & \multicolumn{2}{l}{Recovery error (PID / Static):} & \multicolumn{2}{c}{14.2 / 24.8 ticks} \\
    \midrule
    \multirow{3}{*}{High$\to$Down$\to$Up}
    & Baseline & 16.34 & 16.71 & 15.87 \\
    & Round-trip PID & 11.10 & 3.41 & 13.48 \\
    & Static one-way & 10.10 & 3.13 & 3.10 \\
    \midrule
    & \multicolumn{2}{l}{Recovery error (PID / Static):} & \multicolumn{2}{c}{9.1 / 13.4 ticks} \\
    \bottomrule
  \end{tabular}
  \end{small}
  \end{center}
...
Across all four scenarios, PID reduces Window~3 recovery error by 19--43\% compared to static one-way steering. The high$\to$down$\to$up pitch scenario shows the weakest recovery (40\% within $1\sigma$), likely because the model's distributional priors resist returning to high-register content after sustained low-pitch generation.
