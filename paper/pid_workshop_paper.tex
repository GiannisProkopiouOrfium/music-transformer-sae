...
Our evaluation uses a single model (MMT) and dataset (SOD); gain portability across architectures remains untested. We hypothesize that the $8{\times}$ $K_i$ asymmetry primarily reflects MMT's learned priors rather than universal musical properties, though validating this requires cross-architecture studies. Duration-up steering degrades scale consistency; matched-$\lambda$ analysis suggests both the SAS vector and PID's dynamic trajectory contribute (\cref{app:matched_lambda}). The trajectory-induced component arises because within-sequence $\lambda$ variation generates tokens under inconsistent steering magnitudes, disrupting pitch--duration coupling. Piecewise-constant $\lambda$ scheduling---holding $\lambda(t)$ fixed within musical phrases and updating only at phrase boundaries---would decouple the PID adaptation rate from the token-level generation, potentially recovering static SAS's scale consistency while retaining PID's smooth onset; we leave empirical validation to future work. Sample sizes ($n{=}40$) are modest and perceptual validation is absent. Anti-windup clamping ($I_\text{max}$) and $\lambda_\text{max}$ bounds provide practical stability, though formal OOD guarantees are lacking. Future work includes adaptive gain scheduling, integration with plausibility-gated methods~\cite{kang2026directer}, cross-architecture validation, and SAE variants such as RouteSAE~\cite{shi2025routesae} that may reduce Top-K threshold sensitivity.
...
Temporal PID's per-token overhead relative to static SAS consists of three lightweight operations: (1)~computing the error signal $e(t)$ via a mean over $N{=}32$ pre-indexed feature activations (negligible), (2)~the PID control law---three scalar multiply-adds plus two clamping operations (\cref{eq:pid_temporal}), and (3)~updating the integral accumulator $I(t)$. Both static SAS and temporal PID share the dominant cost: SAE encoding ($512 {\to} 4096$), Top-K re-sparsification, and SAE decoding ($4096 {\to} 512$) at the steered layer, plus the reconstruction correction $\Delta$. The PID-specific computation adds $\mathcal{O}(N + 1)$ scalar operations per token atop this shared SAE forward/backward pass, which itself is $\mathcal{O}(d_\text{sparse})$ with $d_\text{sparse}{=}4096$. In practice, the controller overhead is $<\!1\%$ of the per-token generation time, as autoregressive sampling and attention computation dominate. The key cost difference between steered and unsteered generation is the SAE encode/decode pass, which is shared by both static SAS and PID; temporal PID adds negligible marginal cost beyond static SAS.
