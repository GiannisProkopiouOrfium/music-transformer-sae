    ...
    for scenario, scenario_results in by_scenario.items():
        ...
        for r in scenario_results:
            ...
            if baseline_pitch is not None and baseline_duration is not None:
                ...
                duration_success = (alpha_d > 0) == (
                    duration_rating > baseline_duration
                )
        ...
        if rho_duration is not None:
            logging.info(
                f"  Duration Spearman ρ: {rho_duration:.3f} (p={p_duration:.4f})"
            )
    ...
    for concept in ["pitch", "duration"]:
        ...
        for direction in ["high_to_low", "low_to_high"]:
            dir_results = [
                ...
                and r.get("override_direction") == direction
            ]
            ...
            for alpha, successes in sorted(by_alpha.items()):
                ...
                logging.info(
                    f"  α={alpha:+.1f}: OSR={osr:.1%} ({sum(successes)}/{len(successes)})"
                )
            ...
            if len(by_alpha) > 2:
                ...
                abs_alphas = (
                    np.abs(alphas[alphas != 0.0]) if any(alphas != 0.0) else alphas
                )
...
def analyze_category_4(results: List[dict], config: dict) -> dict:
    ...
    expected_directions = {
        "highpitchshortdurationtolowlong": {"pitch": "DOWN", "duration": "LONGER"},
        "highpitchlongdurationtolowshort": {"pitch": "DOWN", "duration": "SHORTER"},
        "lowpitchshortdurationtohighlong": {"pitch": "UP", "duration": "LONGER"},
        "lowpitchlongdurationtohighshort": {"pitch": "UP", "duration": "SHORTER"},
    }
    ...
    for scenario, scenario_results in by_scenario.items():
        ...
        for r in scenario_results:
            ...
            if pitch_dir and duration_dir and expected_pitch and expected_duration:
                dual_success = (pitch_dir == expected_pitch) and (
                    duration_dir == expected_duration
                )
        ...
        logging.info(f"  Samples: {len(scenario_results)}")
        logging.info(
            f"  Pitch OSR: {pitch_osr:.1%} ({sum(pitch_successes)}/{len(pitch_successes)})"
        )
        logging.info(
            f"  Duration OSR: {duration_osr:.1%} ({sum(duration_successes)}/{len(duration_successes)})"
        )
        logging.info(
            f"  Dual OSR: {dual_osr:.1%} ({sum(dual_successes)}/{len(dual_successes)})"
        )
...
def generate_latex_tables(analysis: dict, output_dir: pathlib.Path):
    ...
    with open(output_dir / "table_category1.tex", "w") as f:
        ...
        for concept in ["pitch", "duration"]:
            ...
            f.write(
                f"{concept.capitalize()} & {n} & {success_str} & {rho_str} & {p_str} \\\\\n"
            )
    ...
    with open(output_dir / "table_category2.tex", "w") as f:
        ...
        for scenario, data in sorted(scenarios.items()):
            ...
            f.write(
                f"{scenario_name} & {n} & {success_str} & {rho_p_str} & {rho_d_str} \\\\\n"
            )
...
def generate_figures(analysis: dict, output_dir: pathlib.Path, config: dict):
    ...
    for idx, concept in enumerate(["pitch", "duration"]):
        ...
        if alphas and ratings:
            ...
            axes[idx].axhline(y=2, color="gray", linestyle="--", label="Neutral")
            axes[idx].set_xlabel("Alpha")
            axes[idx].set_ylabel("Music Flamingo Rating (1-3)")
            axes[idx].set_title(f"{concept.capitalize()} Control")
            axes[idx].set_ylim(0.5, 3.5)  # Set appropriate y-axis range
...
def main():
    ...
    by_category = {
        "category_1": [
            r for r in all_results if r.get("category") == "unconditional_single"
        ],
        "category_2": [
            r for r in all_results if r.get("category") == "unconditional_dual"
        ],
        "category_3": [
            r for r in all_results if r.get("category") == "conditional_single"
        ],
        "category_4": [
            r for r in all_results if r.get("category") == "conditional_dual"
        ],
    }
