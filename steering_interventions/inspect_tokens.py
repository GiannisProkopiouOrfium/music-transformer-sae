    ...
    try:
        ...
        if len(tokens.shape) == 2:
            ...
            for dim_idx in range(n_dims):
                ...
                status = "✓" if vocab_size is None or max_val < vocab_size else "✗ OUT OF BOUNDS!"
                vocab_info = f"(vocab: {vocab_size})" if vocab_size else ""
                
                print(f"     [{dim_idx}] {dim_name:12s}: [{min_val:6d}, {max_val:6d}] {vocab_info} {status}")
                
                if vocab_size and max_val >= vocab_size:
                    print(f"         ⚠️  ERROR: max value {max_val} >= vocab size {vocab_size}")
                    ...
                    if len(bad_indices) > 0:
                        print(f"         First few: {tokens[bad_indices[:5], dim_idx].tolist()}")
...
def main():
    ...
    if args.segment_file and args.segment_file.exists():
        ...
        print(f"\n{'=' * 80}")
        print(f"Summary: Successfully inspected {success_count}/{args.sample_files} files")
