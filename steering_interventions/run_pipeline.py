...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run complete steering intervention pipeline"
    )
    parser.add_argument(
        "--concept", type=str, default="velocity", help="Concept to use"
    )
    parser.add_argument(
        "--n_beats",
        type=int,
    ...
    parser.add_argument(
        "--skip_curation", action="store_true", help="Skip data curation step"
    )
    parser.add_argument(
        "--skip_extraction", action="store_true", help="Skip activation extraction step"
    )
    parser.add_argument(
        "--skip_calculation",
        action="store_true",
    ...
    logging.info(
        f"  - {config.OUTPUT_DIR / 'evaluation' / f'{args.concept}_analysis.json'}"
    )
    logging.info(f"  - {config.OUTPUT_DIR / 'evaluation' / f'{args.concept}_plot.png'}")
    logging.info(
        f"  - {config.OUTPUT_DIR / 'evaluation' / 'samples' / 'alpha_*' / '*.mid'}"
    )
    logging.info("")
