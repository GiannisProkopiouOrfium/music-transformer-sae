...
def detect_key_from_segment(
    tracks: List[Dict], start_beat: int, n_beats: int, resolution: int = 12, min_notes: int = 10
) -> Tuple[str, str, float, int]:
...
def analyze_distribution(
    ...
    for json_path in tqdm(json_files, desc="Processing files"):
        if n_beats is None:
            # Analyze whole song
            mode, tonic, confidence, num_notes = detect_key_from_json(json_path, min_notes)
            all_results.append({
                "file": json_path.name,
                "mode": mode,
                "tonic": tonic,
    ...
    for threshold in confidence_thresholds:
        ...
        print(f"  Total segments:  {total_filtered:,} ({100*total_filtered/total:.1f}% of all)")
        print(f"  Major:           {major_filtered_count:,} ({100*major_filtered_count/total_filtered:.1f}%)" if total_filtered > 0 else "  Major: 0")
        print(f"  Minor:           {minor_filtered_count:,} ({100*minor_filtered_count/total_filtered:.1f}%)" if total_filtered > 0 else "  Minor: 0")
        
        if total_filtered > 0:
            ratio = major_filtered_count / minor_filtered_count if minor_filtered_count > 0 else float('inf')
    ...
    for threshold in confidence_thresholds:
        ...
        if minor_filtered_count > 0 and major_filtered_count > 0:
            ...
            if imbalance < best_balance and len(filtered) > 100:  # Need at least 100 samples
    ...
    sorted_tonics = sorted(
        tonic_counts.items(),
        key=lambda x: x[1]["major"] + x[1]["minor"],
        reverse=True
    )[:10]
    ...
    for tonic, counts in sorted_tonics:
        ...
        print(f"  {tonic:>3}: {total:>4} ({counts['major']:>3} major, {counts['minor']:>3} minor)")
...
def main():
    ...
    confidence_thresholds = [float(t.strip()) for t in args.confidence_thresholds.split(",")]
