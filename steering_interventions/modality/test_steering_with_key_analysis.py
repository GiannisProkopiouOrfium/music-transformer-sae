...
def classify_transition_type(
    tonic1: str, mode1: str, tonic2: str, mode2: str
) -> str:
...
def evaluate_quality_metrics(tokens: np.ndarray, encoding: Dict) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "groove_consistency": muspy.groove_consistency(music, 4 * music.resolution) * 100,
        }
    except Exception as e:
...
def find_extreme_modality_songs(
    ...
    if major_songs:
        logging.info(f"  Examples: {[f'{get_key_string(t, m)} ({c:.3f})' for _, t, m, c in major_songs[:3]]}")
    
    logging.info(f"Found {len(minor_songs)} high-confidence minor songs")
    if minor_songs:
        logging.info(f"  Examples: {[f'{get_key_string(t, m)} ({c:.3f})' for _, t, m, c in minor_songs[:3]]}")
...
def conditioned_generate_and_evaluate(
    ...
    for i, (filepath, cond_tonic, cond_mode, cond_confidence) in enumerate(song_list):
        ...
        for alpha in alphas:
            ...
            tonic_interval, interval_name = calculate_tonic_interval(cond_tonic, gen_tonic)
            ...
            logging.info(
                f"    Generated: {gen_key} (conf: {gen_confidence:.3f})"
            )
            logging.info(
                f"    Transition: {key_transition}"
            )
            logging.info(
                f"    Type: {transition_type}, Interval: {interval_name}"
            )
            logging.info(
...
def analyze_key_transitions(results: List[Dict]) -> Dict:
    ...
    for r in confident:
        ...
        if "degradation" in r:
            score = r["generated_confidence"] * 100 - r["degradation"]["total_degradation"]
        else:
...
def rank_by_key_transition(results: List[Dict]) -> List[Dict]:
...
def main():
    parser = argparse.ArgumentParser(description="Test conditioned modality steering with key analysis")
    parser.add_argument(
    ...
    transition_matrices = {}
    for alpha in alphas:
        transition_matrices[f"alpha_{alpha}"] = create_transition_matrix(all_results, alpha)

    # Rank by key transition quality
    logging.info("\nRanking by key transition clarity and interest...")
    ranked = rank_by_key_transition(all_results)
    ...
    sorted_intervals = sorted(
        transition_analysis["tonic_intervals"].items(),
        key=lambda x: x[1],
        reverse=True
    )
    ...
    print(f"  Parallel keys (same tonic): {len(relationship_analysis['parallel_keys'])}")
    ...
    if relationship_analysis['common_tonic_shifts_positive']:
        ...
        for interval, count in list(relationship_analysis['common_tonic_shifts_positive'].items())[:5]:
            print(f"    {interval}: {count}")

    if relationship_analysis['common_tonic_shifts_negative']:
        print("\n  Common tonic shifts (negative α):")
        for interval, count in list(relationship_analysis['common_tonic_shifts_negative'].items())[:5]:
            print(f"    {interval}: {count}")
    ...
    for alpha_key in sorted(transition_analysis["transition_by_alpha"].keys()):
        ...
        for t_type, t_data in sorted(
            data["transition_types"].items(),
            key=lambda x: x[1]["count"],
            reverse=True
        ):
            print(f"  {t_type:<20} {t_data['count']:>3} ({t_data['percentage']:>5.1f}%)")
