...
def quick_sae_analysis(model_path, activations_path):
    ...
    print(
        f"✅ SAE Architecture: {checkpoint['input_dim']} -> {checkpoint['config']['hidden_dim']}"
    )
    ...
    with h5py.File(activations_path, "r") as f:
        ...
        print(
            f"   Loaded {activations.shape[0]} samples, {activations.shape[1]} features"
        )
    ...
    moderate_features = np.sum(
        (activation_frequencies >= 0.01) & (activation_frequencies < 0.1)
    )
    ...
    print(
        f"   Dead features (0%): {dead_features} ({dead_features/len(activation_frequencies)*100:.1f}%)"
    )
    print(
        f"   Rare features (<1%): {rare_features} ({rare_features/len(activation_frequencies)*100:.1f}%)"
    )
    print(
        f"   Moderate features (1-10%): {moderate_features} ({moderate_features/len(activation_frequencies)*100:.1f}%)"
    )
    print(
        f"   Active features (>10%): {active_features} ({active_features/len(activation_frequencies)*100:.1f}%)"
    )
    ...
    print(
        f"   Average activations per sample: {np.mean(np.sum(all_hidden > 0, axis=1)):.1f}"
    )
    ...
    print(
        f"   Estimated total high correlations: {high_corr_count * (len(activation_frequencies)/sample_features)**2:.0f}"
    )
