...
def generate_dual_static_sas(
    ...
    from sparse_steering.dual_steering.sparse_vector_composer import (
        SparseVectorComposer,
    )
    from sparse_steering.dual_steering.dual_steered_generator import (
        ExpandedKDualSASSteeringHook,
    )
    ...
    p_dict = {
        target_layer: (
            pitch_vector
            if isinstance(pitch_vector, np.ndarray)
            else pitch_vector.numpy()
        )
    }
    d_dict = {
        target_layer: (
            duration_vector
            if isinstance(duration_vector, np.ndarray)
            else duration_vector.numpy()
        )
    }
    composer = SparseVectorComposer(p_dict, d_dict)
    combined = composer.compose(
        lambda_pitch, lambda_duration, strategy="gram_schmidt_ek2"
    )
...
def generate_dual_diffmean(
    ...
    sys.path.insert(
        0, str(pathlib.Path(__file__).parent.parent / "steering_interventions")
    )
    ...
    combined = composer.compose(
        alpha_pitch, alpha_duration, strategy="gram_schmidt_pitch"
    )
    ...
    generated = generator.generate(
        start_tokens,
        seq_len,
        alpha=1.0,  # alpha baked into composed vector
        eos_token=eos,
        temperature=config_pid.TEMPERATURE,
        filter_logits_fn="top_k",
        filter_thres=config_pid.FILTER_THRESHOLD,
...
def compute_dual_success(
    metrics_list: List[dict], baseline_metrics: List[dict], pitch_dir: str, dur_dir: str
) -> float:
    """Compute dual success rate: fraction where BOTH attributes shift correctly."""
    n_success = 0
    for m, b in zip(metrics_list, baseline_metrics):
        pitch_ok = (
            (m["average_pitch"] > b["average_pitch"])
            if pitch_dir == "up"
            else (m["average_pitch"] < b["average_pitch"])
        )
        dur_ok = (
            (m["average_duration"] > b["average_duration"])
            if dur_dir == "up"
            else (m["average_duration"] < b["average_duration"])
        )
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["unconditioned", "conditioned"], default="unconditioned"
    )
    parser.add_argument(
        "--n_samples", type=int, default=20, help="Samples for unconditioned"
    )
    parser.add_argument(
        "--n_songs", type=int, default=10, help="Songs per scenario for conditioned"
    )
    parser.add_argument(
        "--n_per_song", type=int, default=2, help="Reps per song for conditioned"
    )
    ...
    dm_pitch_path = (
        config_pid.STEERING_VECTORS_DIR / "average_pitch_steering_vectors.pt"
    )
    dm_dur_path = (
        config_pid.STEERING_VECTORS_DIR / "average_duration_steering_vectors.pt"
    )
    ...
    try:
        ...
        logger.warning(
            f"Could not load DiffMean vectors: {e}. Skipping DiffMean comparison."
        )
...
def _run_unconditioned(
    ...
    pid_metrics["avg_lambda_pitch"] = float(
        np.mean([np.mean(d["pitch_lambda_trajectory"]) for d in pid_diags])
    )
    pid_metrics["avg_lambda_duration"] = float(
        np.mean([np.mean(d["duration_lambda_trajectory"]) for d in pid_diags])
    )
    ...
    for i in range(args.n_samples):
        seq = generate_dual_static_sas(
            ...
            duration_sas,
            args.target_layer,
            args.sas_lambda_pitch,
            args.sas_lambda_duration,
            start,
            args.seq_len,
            device,
        )
...
def _run_conditioned(
    ...
    if args.song_offset > 0:
        ...
        logger.info(
            f"Song offset={args.song_offset}: skipping first {args.song_offset} songs per group"
        )
    ...
    for scenario in scenarios:
        ...
        n_use = min(
            len(scenario["pitch_songs"]), len(scenario["dur_songs"]), args.n_songs
        )
        ...
        for song_idx in range(n_use):
            ...
            for rep_idx in range(args.n_per_song):
                ...
                # 4. Baseline
                seq_b = generate_baseline(
                    model, encoding, cond, args.continuation_len, device
                )
        ...
        # Dual success
        base_ml = [
            compute_generation_metrics(s, encoding) for s in method_seqs["baseline"]
        ]
        ...
        if "diffmean" in method_seqs:
            ml = [
                compute_generation_metrics(s, encoding) for s in method_seqs["diffmean"]
            ]
        ...
        if pid_diags:
            scenario_results["dual_pid"]["avg_lambda_pitch"] = float(
                np.mean(
                    [
                        np.mean(d["pitch_lambda_trajectory"])
                        for d in pid_diags
                        if d["pitch_lambda_trajectory"]
                    ]
                )
            )
            scenario_results["dual_pid"]["avg_lambda_duration"] = float(
                np.mean(
                    [
                        np.mean(d["duration_lambda_trajectory"])
                        for d in pid_diags
                        if d["duration_lambda_trajectory"]
                    ]
                )
            )
...
def _print_summary(results: dict, title: str):
    ...
    print(f"{'='*80}")
    print(
        f"{'Method':<22} {'Pitch':<8} {'Dur':<8} {'Entropy':<9} "
