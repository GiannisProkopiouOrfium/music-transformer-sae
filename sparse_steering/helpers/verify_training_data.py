        ...
        with h5py.File(data_path, "r") as f:
            ...
            for layer_idx in range(num_layers):
                ...
                if abs(mean) > 100 or std > 100:
                    print(
                        f"✗ Layer {layer_idx}: Suspicious stats (mean={mean:.2f}, std={std:.2f})"
                    )
                    all_checks_passed = False
                    continue
            ...
            if len(set(sample_counts)) != 1:
                ...
                print(
                    f"✓ Consistent sample count: {sample_counts[0]} across all layers"
                )
            ...
            print(
                f"✓ Estimated training time: ~{estimated_hours:.1f} hours (very rough)"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Verify SAE training data quality")
    parser.add_argument(
        "--data_path",
        type=pathlib.Path,
        default=None,
    ...
    if args.data_path is None:
        base_dir = (
            ...
            / "exp/sod/sparse_steering/sae_training_data"
        )
        if args.quick_test:
