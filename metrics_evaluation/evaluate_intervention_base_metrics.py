...
def evaluate_sequence_base_metrics(sequence: np.ndarray, encoding: Dict) -> Dict[str, float]:
    ...
    try:
        ...
        try:
            metrics["groove_consistency"] = muspy.groove_consistency(music, 4 * music.resolution)
        except Exception as e:
...
def extract_condition_info(filename: str) -> Dict[str, Any]:
    ...
    if filename.startswith("baseline_"):
        return {"condition_type": "baseline", "strength": 0.0, "condition_name": "baseline"}
        ...
        try:
            ...
            return {"condition_type": "addition", "strength": strength, "condition_name": f"add_{strength:+.1f}"}
        except ValueError:
            return {"condition_type": "unknown", "strength": 0.0, "condition_name": filename}
    elif filename.startswith("ablation_"):
        return {"condition_type": "ablation", "strength": 0.0, "condition_name": "ablation"}
    else:
        return {"condition_type": "unknown", "strength": 0.0, "condition_name": filename}


def evaluate_intervention_directory(intervention_dir: Path, encoding: Dict, logger: logging.Logger) -> Dict:
    ...
    for tensor_file in tensor_files:
        ...
        try:
            ...
            if "error" not in metrics:
                logger.info(f"    ✅ pitch_class_entropy: {metrics['pitch_class_entropy']:.4f}")
                logger.info(f"    ✅ scale_consistency: {metrics['scale_consistency']:.4f}")
                logger.info(f"    ✅ groove_consistency: {metrics['groove_consistency']:.4f}")
            else:
    ...
    valid_conditions = {k: v for k, v in results["conditions"].items() 
                       if "metrics" in v and "error" not in v["metrics"]}
    
    if valid_conditions:
        # Compute mean and std for each metric
        for metric_name in ["pitch_class_entropy", "scale_consistency", "groove_consistency"]:
            values = [v["metrics"][metric_name] for v in valid_conditions.values() 
...
def create_overall_summary(all_results: List[Dict]) -> Dict:
            ...
            if "metrics" in condition_data and "error" not in condition_data["metrics"]:
                ...
                for metric_name, metric_value in metrics.items():
                    if not isinstance(metric_value, str) and not np.isnan(metric_value):
                        ...
                        condition_metrics[condition_name][metric_name].append(metric_value)
    ...
    for condition_name, metrics_dict in condition_metrics.items():
        for metric_name, values in metrics_dict.items():
            if values:
                summary["condition_summaries"][condition_name]["metrics"][metric_name] = {
                    "mean": float(np.mean(values)),
                    "std": float(np.std(values)),
                    "count": len(values)
                }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate intervention results using base MusPy metrics")
    parser.add_argument(
        "--interventions-dir",
        type=Path,
    ...
    if not intervention_dirs:
        logger.error(f"❌ No intervention directories found in {args.interventions_dir}")
        return False
    ...
    if args.layers or args.features:
        ...
        logger.info(f"Filtered to {len(intervention_dirs)} directories based on layer/feature criteria")
    
    # Evaluate each intervention directory
    logger.info("\n🎯 STARTING EVALUATION")
    all_results = []
    
    for i, intervention_dir in enumerate(tqdm.tqdm(intervention_dirs, desc="Evaluating interventions")):
        ...
        try:
            ...
            result_file = args.output_dir / f"{intervention_dir.parent.name}_{intervention_dir.name}_base_metrics.json"
    ...
    comprehensive_results = {
        "evaluation_info": {
            ...
            "metrics_evaluated": ["pitch_class_entropy", "scale_consistency", "groove_consistency"]
        },
    ...
    for layer, layer_data in overall_summary["layer_summaries"].items():
        ...
        for metric_name, stats in layer_data["metrics"].items():
            logger.info(f"    {metric_name}: mean={stats['mean']:.4f}, std={stats['std']:.4f}")
    
    logger.info("\n📋 BY CONDITION:")
    for condition_name, condition_data in overall_summary["condition_summaries"].items():
