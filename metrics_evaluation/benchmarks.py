...
def compute_confidence_interval(values: list, confidence: float = 0.95) -> Tuple[float, float, float]:
    """
...
def calculate_relative_performance(observed_mean: float, benchmark_name: str, 
                                 metric_name: str) -> Dict[str, float]:
    """Calculate relative performance vs paper benchmarks."""
    
    if benchmark_name not in PAPER_BENCHMARKS or metric_name not in PAPER_BENCHMARKS[benchmark_name]:
    ...
    if metric_name in ["scale_consistency", "groove_consistency"] and benchmark_mean < 1.0:
        benchmark_mean *= 100
...
def assess_statistical_significance_vs_benchmark(observed_values: list, 
                                               benchmark_name: str, 
                                               metric_name: str,
...
def _extract_baseline_values(results_dict: Dict, controlled_baseline_name: str, key_metrics: list) -> Dict:
    ...
    if controlled_baseline_name in results_dict:
        ...
        for metric in key_metrics:
            if "metrics" in baseline_condition and metric in baseline_condition["metrics"]:
    ...
    for metric_name in key_metrics:
        if metric_name in baseline_values:
            ...
            for benchmark_name in ["ground_truth", "original_mmt", "mmm", "remi_plus"]:
                comparison = calculate_relative_performance(baseline_val, benchmark_name, metric_name)
                comparison_results[metric_name][benchmark_name] = comparison
    
    return comparison_results


def _calculate_single_metric_effect(baseline_val: float, intervention_val: float) -> Dict:
    """Calculate intervention effect for a single metric."""
    percent_change = (intervention_val - baseline_val) / baseline_val * 100 if baseline_val != 0 else 0.0
    ...
    return {
        "intervention_effects": {
            ...
            "effect_interpretation": "improvement" if percent_change > 0 else "degradation"
        },
        "effect_sizes": {
            "cohens_d": cohens_d,
...
def _analyze_intervention_effects(results_dict: Dict, controlled_baseline_name: str, 
                                baseline_values: Dict, key_metrics: list) -> Dict:
    """Analyze intervention effects relative to controlled baseline."""
    intervention_analysis = {}
    
    for condition_name, condition_data in results_dict.items():
        if condition_name == controlled_baseline_name or "metrics" not in condition_data:
            continue
        ...
        for metric_name in key_metrics:
            if (metric_name in baseline_values and 
                metric_name in condition_data["metrics"] and
                not np.isnan(condition_data["metrics"][metric_name])):
                
                baseline_val = baseline_values[metric_name]
                intervention_val = condition_data["metrics"][metric_name]
                
                # Calculate effects for this metric
                effects = _calculate_single_metric_effect(baseline_val, intervention_val)
                condition_analysis["intervention_effects"][metric_name] = effects["intervention_effects"]
                condition_analysis["effect_sizes"][metric_name] = effects["effect_sizes"]
...
def create_benchmark_comparison_summary(results_dict: Dict, 
                                      controlled_baseline_name: str = "baseline") -> Dict[str, Any]:
    """
    ...
    baseline_values = _extract_baseline_values(results_dict, controlled_baseline_name, key_metrics)
...
def get_benchmark_reference(benchmark_name: str, metric_name: str) -> Dict[str, Any]:
    ...
    if benchmark_name in PAPER_BENCHMARKS and metric_name in PAPER_BENCHMARKS[benchmark_name]:
...
def format_benchmark_table() -> str:
    ...
    for model_name, model_data in PAPER_BENCHMARKS.items():
        ...
        sc = format_metric_with_ci(**model_data["scale_consistency"], is_percentage=True)
        gc = format_metric_with_ci(**model_data["groove_consistency"], is_percentage=True)
