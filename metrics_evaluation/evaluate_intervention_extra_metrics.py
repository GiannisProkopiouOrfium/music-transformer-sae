...
try:
    from benchmarks import (
        calculate_cohens_d, interpret_cohens_d,
        compute_confidence_interval, CONTROLLED_GENERATION_PARAMS
    )
...
def compute_note_density_metrics(music: muspy.Music) -> Dict[str, float]:
    ...
    duration_beats = music.get_end_time() / music.resolution if music.resolution > 0 else 1
    
    # Notes per beat
    notes_per_beat = total_notes / duration_beats if duration_beats > 0 else 0
    
    # Note density (notes per quarter note equivalent)
    note_density = total_notes / (music.get_end_time() / music.resolution) if music.get_end_time() > 0 else 0
    ...
    try:
        ...
        for track in music.tracks:
            for note in track.notes:
                ...
                all_pitches_by_time[time_bucket].append(note.pitch % 12)  # Pitch classes
        ...
        for pitches in all_pitches_by_time.values():
            if len(pitches) > 1:
                ...
                consonance = 1.0 - (len(unique_pitches) - 1) / 11.0  # Normalize to [0, 1]
...
def compute_musical_coherence_metrics(music: muspy.Music) -> Dict[str, float]:
    ...
    if len(phrase_lengths) > 1:
        structural_coherence = 1.0 / (1.0 + np.std(phrase_lengths) / np.mean(phrase_lengths))
    else:
    ...
    melodic_coherence = float(np.mean(melodic_coherence_scores)) if melodic_coherence_scores else 0.0
...
def evaluate_sequence_extended_metrics(sequence: np.ndarray, encoding: Dict) -> Dict[str, Any]:
...
def extend_results_with_detailed_metrics(base_results: Dict, encoding: Dict, logger: logging.Logger) -> Dict:
    ...
    extended_results["evaluation_info"]["metrics_evaluated"].extend([
        "note_density_metrics", "pitch_range_metrics", "rhythmic_consistency_metrics",
        "harmonic_progression_metrics", "musical_coherence_metrics"
    ])
    ...
    for i, result in enumerate(extended_results["detailed_results"]):
        ...
        logger.info(f"📊 [{i+1}/{len(extended_results['detailed_results'])}] Processing Layer {layer}, Feature {feature_id}")
        ...
        for condition_name, condition_data in result["conditions"].items():
            if "tensor_file" in condition_data:
                try:
                    ...
                    extended_metrics = evaluate_sequence_extended_metrics(seq_np, encoding)
                    ...
                    logger.error(f"  ❌ {condition_name}: Failed to compute extended metrics: {e}")
...
def create_extended_summary(extended_results: Dict) -> Dict:
    ...
    for result in extended_results["detailed_results"]:
        ...
        for condition_name, condition_data in result["conditions"].items():
            if "extended_metrics" in condition_data and "error" not in condition_data["extended_metrics"]:
                    ...
                    if isinstance(category_metrics, dict) and "error" not in category_metrics:
                        for metric_name, metric_value in category_metrics.items():
                            if not isinstance(metric_value, str) and not np.isnan(metric_value):
                                ...
                                all_extended_metrics[full_metric_name]["all"].append(metric_value)
                                condition_extended_metrics[condition_name][full_metric_name].append(metric_value)
                                layer_extended_metrics[layer][full_metric_name].append(metric_value)
...
def _compute_effect_size_for_condition(intervention_metrics: Dict, baseline_values: Dict) -> Dict:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate interventions using extended intervention-specific metrics")
    parser.add_argument(
        "--base-results",
        type=Path,
    ...
    extended_results = extend_results_with_detailed_metrics(base_results, encoding, logger)
    ...
    for metric_name, stats in extended_summary["extended_metrics_summary"].items():
        logger.info(f"  {metric_name}: mean={stats['mean']:.4f}, std={stats['std']:.4f} (n={stats['count']})")
