...
class RealContrastLiMuFExtractor:
    ...
    def extract_contrast_limuf(
        ...
        desc_a = self.interpretations.get(str(feature_a_id), {}).get("interpretation", f"Feature {feature_a_id}")
        desc_b = self.interpretations.get(str(feature_b_id), {}).get("interpretation", f"Feature {feature_b_id}")
        ...
        with torch.no_grad():
            for i in tqdm(range(0, n_samples, batch_size), desc="SAE encoding"):
                ...
                _, sae_features = self.sae_model(batch_activations)  # [batch, n_features]
        ...
        print(f"📊 Feature {feature_a_id} fires in {n_a_samples} samples (>{activation_threshold})")
        print(f"📊 Feature {feature_b_id} fires in {n_b_samples} samples (>{activation_threshold})")
        
        if n_a_samples < min_samples:
            print(f"❌ Not enough samples for Feature {feature_a_id} ({n_a_samples} < {min_samples})")
            return None
        
        if n_b_samples < min_samples:
            print(f"❌ Not enough samples for Feature {feature_b_id} ({n_b_samples} < {min_samples})")
            return None
        
        # Step 3: Get corresponding residual stream activations
        activations_when_a_fires = self.activations[high_a_mask]  # [n_a_samples, d_model]
        activations_when_b_fires = self.activations[high_b_mask]  # [n_b_samples, d_model]
    ...
    def save_contrast_limuf(
        ...
        if contrast_name is None:
            contrast_name = f"feature_{stats['feature_a_id']}_vs_{stats['feature_b_id']}"
...
def main():
    ...
    parser.add_argument(
        ...
        default="layer_interpretations/enhanced_diversity_report_layer3.json",
